        ...
        child: Stack(
          children: [
            ...
            GestureDetector(
              ...
              onTap: () {
                if (provider.videoPlayervideos.value.isInitialized) {
                  if (provider.videoPlayervideos.value.isPlaying) {
                    provider.videoPlayervideos.pause();
                    provider.setIsplayingVideos();
                  } else {
                    provider.videoPlayervideos.play();
                    provider.setIsplayingVideos();
            ...
            Positioned(
                ...
                child: Container(
                  ...
                  child: getButtonList(
                      ...
                      userInfo,
                      uid,
                      provider.videoPlayervideos,
                      provider.isplayingVideros,
                      widget.isprofile),
                )),
          ],
        ),
...
class VideoPalyer1 extends StatefulWidget {
  VideoPalyer1(
      {Key key,
      @required this.url,
      this.provider,
  ...
  RecommendProvider provider;
  @override
  _VideoPalyer1State createState() => _VideoPalyer1State(
      this.url, this.provider, this.uid, this.index, this.isprofile);
}

class _VideoPalyer1State extends State<VideoPalyer1> {
  bool isprofile;
  String url;
  String uid;
  ...
  void initState() {
    ...
    provider.initIsplayingVideos();
    videoPlayer = VideoPlayerController.network(url);
    provider.setVideoPalyerVideos(videoPlayer);
  ...
  videobutton() {
    ...
    return Align(
      ...
      child: Offstage(child: item, offstage: provider.isplayingVideros),
      //必须用变化的
    );
  }
  ...
  // double ResetTVidersize(double videoSize, double phoneW, double phoneH) {
  //   double resize;
  //   if (videoSize > phoneSize) {
  //     resize = videoSize * videoSize / phoneSize;
  //   } else {
  //     resize = videoSize;
  //   }
  //   print("sesize$resize");
